...
class HomeController extends Controller
{
    public function editor($business){
        // if(session()->get('bus_id')){
            ...
            return view('editor', compact('business', 'bname'));
        // }
    }
        ...
        if(!empty($buss)) {
            ...
            foreach($business_data as $busdata){
                if($busdata->template == 1){
                    return view('templaes.template-one', ['bus' => $buss,'user' => $user,'buscon' => $buscon, 'connect'=>$connect, 'bname'=>$username]);
                }
                elseif($busdata->template == 2){
                    return view('templaes.template-two', ['bus' => $buss,'user' => $user,'buscon' => $buscon, 'connect'=>$connect, 'bname'=>$username]);
                }
                elseif($busdata->template == 3){
                    return view('templaes.template-three', ['bus' => $buss,'user' => $user,'buscon' => $buscon, 'connect'=>$connect, 'bname'=>$username]);
                }
                else{
                    return view('templaes.template-one', ['bus' => $buss,'user' => $user,'buscon' => $buscon, 'connect'=>$connect, 'bname'=>$username]);
                }
